...
| File | What it is |
|---|---|
| `pso-poseidon-X.Y.Z.crate` | The byte-identical .crate uploaded to crates.io. |
| `pso-poseidon-X.Y.Z.crate.sig` | cosign blob signature (raw, base64). |
| `pso-poseidon-X.Y.Z.crate.pem` | Fulcio-issued ephemeral signing cert (PEM). |
...
- A compromise of `github.com/psonet/pso-poseidon` itself (an attacker with push access to `main` can edit the workflow to remove or weaken signing).
...
- Existing (pre-cutoff) releases. Those are **not** retroactively signed — see [the spec's D10 rationale](https://github.com/psonet/pso-poseidon/blob/main/SECURITY.md#retroactive-signing).
...
For security issues in `pso-poseidon` itself (not the signing pipeline), open a [private security advisory](https://github.com/psonet/pso-poseidon/security/advisories/new) on GitHub. Do not file a public issue.
